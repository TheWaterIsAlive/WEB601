...
function getAllBussnessKnex(req, res) {
    ...
    const { //Establishes a local connection to the database
        knex
    } = req.app.locals
    knex
...
function postBussnessKnex(req, res) {
    ...
    const mandatoryColumnsExists = mandatoryColumns.every(mc => payloadKeys.includes(mc)) // Makes sure that the paylaoad confonforms to the mandator column rules
    if (mandatoryColumnsExists) {
        ...
        return res.status(400).json(`Mandatory Columns are required ${mandatoryColumns}`); //Error for which is for when a manadory column is not filled.
    }
}
...
function putBussnessKnex(req, res) {
    ...
    knex('bussnesses')
        ...
        .then(response => {
            ...
            } else { //These messages dissplay the name of the bussness as part of the error/confirmation messages
